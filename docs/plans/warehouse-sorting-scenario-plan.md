...
| 步骤2a：小车已到达，直接进入发车 | `if_else` 节点查询 RCS 状态，条件成立直接跳过 wait | ✅ 已支持 |
...
flowchart LR
    ...
    S3Q --> S4{小车已到达?}
    S4 -- 是 --> S5[plugin:dispatch_task<br/>挂起等待工人确认发车]
...
- **已到达**：直接进入 `plugin:dispatch_task`，工人确认后发车
- **未到达**：`wait(event=car_arrived)` 挂起；RCS 推送到达事件后，平台 webhook 调用 `POST /runs/{run_id}/resume` 唤醒，再进入 `plugin:dispatch_task`
...
flowchart LR
    ...
    SC2 --> SC2Q[fetch<br/>查询小车是否已到达]
    SC2Q --> SC2C{小车已到达?}
    SC2C -- 是 --> SC4[plugin:dispatch_task<br/>挂起等发车]
    ...
    class T2,SC2,SC2Q,RC1,RC5 fetch
    class T1,T3 state
    class SC2C cond
```
