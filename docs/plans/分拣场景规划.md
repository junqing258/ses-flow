...
flowchart LR
    ...
    W1 -.RCS 推送小车到站<br/>POST /api/rcs/events.-> W1R["/resume carId"]
...
- **等待小车到达**：`wait(event=car_arrived, stationId)` 挂起；RCS 推送到站事件后，平台 webhook 调用 `POST /runs/{run_id}/resume`（携带 `carId`）唤醒；超时走异常分支
- **等待扫码**：`plugin:scan_task` 下发扫码任务到 App，工人扫码后 App 调用 `POST /station/operation/scanBarcode({barcode})`，接口返回 `{items:[{itemId, skuName, url}]}`；该调用同时作为 resume 触发条件，携带 `itemId` 唤醒挂起的 run
- **获取任务/订单信息**：`plugin:get_task_info` 以 resume 传入的 `itemId` 为起点，内部串联两个接口，完成后通过 `statePatch` 写回 run state：
...
- **发车**：`plugin:robot_departure` 调用 `POST /station/operation/robotDeparture`，入参 `{taskId}`，返回 `{result}`；`result` 枚举值：`SUCCESS(0)` 继续流程，`NO_AGV(1)` / `NO_TASK(2)` 走异常分支
