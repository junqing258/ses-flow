...
| 波次绑定目的 chute（分拣格口，可为空） | 无 chute 字段与分配逻辑；chute 为空时需在 RCS 子流程入口随机分配空闲格口 | ❌ 缺失 |
...
| chute 为空时随机分配空闲格口 | 需在子流程入口处 `fetch` 格口服务获取空闲格口，`set_state` 写入 chute | ❌ 需格口服务支持 |
...
flowchart LR
    ...
    R0 -- 否 --> R0F[fetch<br/>GET /chutes/idle<br/>随机取空闲格口]
    R0F --> R0S[set_state<br/>写入 chute]
    R0S --> R4
...
**备注**：上一环节（人工工作台）已确认小车到达，本子流程无需发调度指令，也不等待货物落入格口事件，仅等待卸货确认（翻板抬起）。chute 为空时在子流程入口随机分配空闲格口。需要补的是**RCS 事件接入端点**（统一 webhook 入口 + 按 run_id/request_id 路由到对应 wait 节点）以及**格口服务空闲查询接口**。
...
flowchart LR
    ...
    RC0 -- 否 --> RC0F[fetch<br/>GET /chutes/idle<br/>随机取空闲格口]
    RC0F --> RC0S[set_state<br/>写入 chute]
    RC0S --> RC3[wait<br/>event=unloaded<br/>timeout=30s]
    RC0 -- 是 --> RC3
    ...
    class SC1,P1 plugin
    class SC3,RC3 wait
    class T2,SC2Q,RC0F,RC5 fetch
    class T1,T3,RC0S state
    class SC2C,RC0 cond
```
...
2. 实现格口状态服务 `GET /chutes/{id}/status` + 件数累加 API + `GET /chutes/idle`（随机返回一个空闲格口，供 chute 为空时分配）
3. RCS 提供小车当前位置查询接口（供 `fetch` + `if_else` 判断小车是否已到站）
4. 配置 RCS 回调地址 + 在平台注册业务插件 `baseUrl`（若尚未注册）
5. 波次号、platformId、chute 暂存在 run 的 `state`，不建独立波次对象（验证阶段）；chute 为空时在 RCS 子流程入口由 `fetch /chutes/idle` 动态分配后写入 state
