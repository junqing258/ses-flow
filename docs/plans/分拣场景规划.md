...
幂等校验 + 字段标准化"]
    ...
    C1 --> F1["fetch
GET /wave-rules/evaluate
查询波次规则
← {rule_id, rule_version, platformId}"]
...
{wave_no, platformId, chute}".-> WR["/resume"]
...
| 步骤 | 节点 | 说明 |
|---|---|---|
| 接收订单 | `webhook_trigger` + `code` | 触发入口；code 节点做幂等校验与字段标准化 |
| 查询波次规则 | `fetch GET /wave-rules/evaluate` | 按订单属性（sku/地区/日期）匹配适用规则，返回 `rule_id`、`platformId` 等元信息；规则不内嵌流程，可热更新 |
| 等待波次生成 | `wait(event=wave_formed)` | run 挂起；波次服务在订单池积攒达到阈值后，调用 `POST /runs/{run_id}/resume` 推送 `wave_no` 唤醒；超时走异常分支 |
| 波次激活 | `set_state` + `fetch POST /waves/activate` | 将 `wave_no`/`platformId`/`chute` 写入 run state；调用波次服务将波次状态从 `pending` 切换为 `active` |
| 进入扫码流程 | — | `platformId` 与 `wave_no` 已就绪，后续扫码/RCS 子流程直接读 `{{ state.* }}` |
...
工作站侧为**四步子流程**，以 `stationId` 作为入口标识：

```
开始(stationId) → 等待小车到达 → 等待扫码 → 获取任务/订单信息 → 发车
```

| 能力需求 | 当前支撑 | 状态 |
|---|---|---|
| 步骤1：等待小车到达（RCS 推送到站事件） | `wait` 节点 + `POST /runs/{run_id}/resume` | ✅ 已支持 |
| 步骤2：下发扫码任务 + 接收扫码结果 | `plugin:scan_task` 节点 | ✅ 前提已实现 |
| 步骤3：获取任务/订单信息（插件内串联 scanBarcode→getTaskInfo→lockTask，statePatch 写回） | `plugin:get_task_info` 节点（HTTP 插件） | ❌ 需实现插件服务 |
| 步骤4：发车（插件内调用 robotDeparture，按 result 枚举分支） | `plugin:robot_departure` 节点（HTTP 插件） | ❌ 需实现插件服务 |
| stationId 绑定 wave / platformId | 由上游波次流程写入 run state，本子流程直接读取 | ✅ 已支持 |
| 小车到达事件路由 | RCS → 平台 webhook → 定位到等待中的 run → resume | ❌ 需实现 RCS 事件接入端点 |
| 任务/订单信息写入 run state | 插件通过 `statePatch` 返回（taskId/orderId/orderDetailId/targetId/count） | ✅ 已支持 |
| 工作站并发：多订单共享同一 stationId 的小车 | 无工作站级共享状态，多 run 独立等待 | ❌ 需工作站状态服务 |

**子流程映射到节点**（Mermaid）：

```mermaid
flowchart LR
    ENTRY(["开始\nstationId"]) --> W1[wait<br/>event=car_arrived<br/>timeout=10min]
    W1 -.RCS 推送小车到站<br/>POST /api/rcs/events.-> W1R["/resume agvId"]
    W1R --> S1[plugin:scan_task<br/>挂起等待扫码]
    S1 -."App 调用 POST /station/operation/scanBarcode<br/>{barcode} → {itemId,skuName,url}<br/>触发 /resume {itemId}".-> S1R["/resume itemId"]
    S1R --> P1["plugin:get_task_info<br/>① getTaskInfo(stationId,itemId)<br/>② lockTask(stationId,orderDetailId,completed)<br/>statePatch→taskId/orderId/orderDetailId/targetId/count"]
    P1 --> P2["plugin:robot_departure<br/>robotDeparture(taskId)<br/>result: SUCCESS/NO_AGV/NO_TASK"]
    P2 -- SUCCESS --> OUT([进入 RCS 分拣子流程])
    P2 -- NO_AGV/NO_TASK --> ERR2[异常分支<br/>发车失败]

    W1 -- 超时 --> ERR1[异常分支<br/>小车未到达]

    classDef plugin fill:#fef3c7,stroke:#d97706
    classDef wait fill:#fee2e2,stroke:#dc2626
    classDef err fill:#fecaca,stroke:#991b1b
    class S1,P1,P2 plugin
    class W1 wait
    class ERR1,ERR2 err
```
...
| 节点 | 输入参数 | 来源 | 写出到 state |
|---|---|---|---|
| `wait(car_arrived)` | `stationId` | `{{ state.stationId }}`（上游波次流程写入） | resume 携带 `agvId` → `{{ state.agvId }}` |
| `plugin:scan_task` | `stationId`, `agvId` | `{{ state.stationId }}`, `{{ state.agvId }}` | resume 触发由 scanBarcode 调用携带 `itemId` → `{{ state.itemId }}` |
| `plugin:get_task_info` | `stationId`, `itemId` | `{{ state.stationId }}`, `{{ state.itemId }}` | `statePatch` → `taskId / orderId / orderDetailId / targetId / count` |
| `plugin:robot_departure` | `taskId` | `{{ state.taskId }}` | — |

**各步骤说明**：

- **等待小车到达**：以 `{{ state.stationId }}` 作为事件过滤条件，`wait(event=car_arrived)` 挂起；RCS 推送到站事件后，平台 webhook 调用 `POST /runs/{run_id}/resume`（携带 `agvId`），runner 将 `agvId` 写入 run state；超时走异常分支
- **等待扫码**：`plugin:scan_task` 以 `{{ state.stationId }}`、`{{ state.agvId }}` 为上下文下发扫码任务到 App；工人扫码后 App 调用 `POST /station/operation/scanBarcode({barcode})`，接口返回 `{items:[{itemId, skuName, url}]}`；该调用同时作为 resume 触发条件，携带 `itemId` 唤醒挂起的 run，runner 将 `itemId` 写入 run state
- **获取任务/订单信息**：`plugin:get_task_info` 从 `{{ state.stationId }}`、`{{ state.itemId }}` 读取输入，内部串联两个接口，完成后通过 `statePatch` 写回 run state：
  1. `POST /station/operation/getTaskInfo` — 入参 `{stationId, itemId}` → 返回 `{orderDetailId, count, targetId, orderId}`
  2. `POST /station/operation/lockTask` — 入参 `{stationId, orderDetailId, completed}` → 返回 `{success, taskId}`；`statePatch: {taskId, orderId, orderDetailId, targetId, count}`
- **发车**：`plugin:robot_departure` 从 `{{ state.taskId }}` 读取输入，调用 `POST /station/operation/robotDeparture({taskId})`，返回 `{result}`；`SUCCESS(0)` 继续流程，`NO_AGV(1)` / `NO_TASK(2)` 走异常分支
