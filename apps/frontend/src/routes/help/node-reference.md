...
本页聚焦当前前端编辑器已经对外开放的节点能力，并补充与 Runner 行为有关的使用说明。仅保留运行时兼容的历史节点不在本页展开。

## 如何理解“节点说明”

当前有两层能力需要区分：

1. Runner 运行时可以识别哪些节点定义。
2. 前端编辑器当前把哪些节点开放成可拖拽、可配置的节点。

这两层能力并不完全一致，所以本页只聚焦当前对外开放的编辑器节点。
...
- 人工确认、设备回调、第三方异步通知这类场景，优先通过 `Wait` 节点建模。
