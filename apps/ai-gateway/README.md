...
AI_GATEWAY_RUNNER_BASE_URL=...   # 可选，Docker 部署时可指向容器内 backend 地址

# 可选：Claude Code 可执行文件路径
CLAUDE_CODE_EXECUTABLE=/path/to/claude
```

当 `AI_GATEWAY_RUNNER_BASE_URL` 被设置时，AI Gateway 会优先使用它访问 runner API，而不是请求体里的 `runnerBaseUrl`。这在 Docker 部署中很有用，因为浏览器通常会把 `runnerBaseUrl` 发送成 `http://localhost:6302/runner-api`，容器内需要改走 `http://backend:6302/runner-api`。
