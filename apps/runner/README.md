...
2. 每次循环根据当前节点的 `node_type`，从 `ExecutorRegistry` 里找到对应的 `NodeExecutor` 实现，例如 `fetch / code / shell / wait / task / sub_workflow`。
...
- `wait`、`task`、`sub_workflow` 这类可暂停节点会返回 `Waiting`，引擎会把当前 `run_id`、节点位置、状态、时间线、最后一次 signal 等信息封装成 `resume_state / snapshot`。
...
- 大多数节点都在这个进程内直接执行，例如流程编排、分支判断、`wait`、`task`、`sub_workflow`。其中 `sub_workflow` 也是在当前进程里直接调用子流程引擎，不会额外拉起服务进程。
